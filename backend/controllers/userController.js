...
exports.resetPassword = catchAsycError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(
        "rest password token is invaild or token is expired !  ",
        400
      )
    );
  }
...
exports.updatePassword = catchAsycError(async (req, res, next) => {
  ...
  if (req.body.newPassword !== req.body.confirmPassword) {
    return next(
      new ErrorHandler(
        "new password and confirm pasword did not matched ! ",
        400
      )
    );
  }
...
exports.getSingleUser = catchAsycError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(`User not exist with Id: ${req.pasrms.id}`, 404)
    );
  }
